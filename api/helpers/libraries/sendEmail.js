...
const sendResetPasswordEmailWithToken = async (resetToken,email) => {
  ...
  const resetPasswordUrl = process.env.NODE_ENV === "development" ? `http://localhost:3000/resetpassword?resetPasswordToken=${resetToken}` : `https://password-manager-git-master.ogulcankarayel5.vercel.app/resetPassword?resetPasswordToken=${resetToken}`
  const emailTemplate = `
